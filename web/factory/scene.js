...
export class FactoryScene {
    ...
    // --- Ground & roads ---------------------------------------------------------
    buildGround() {
        const ground = new THREE.Mesh(new THREE.BoxGeometry(40, 1, 36), new THREE.MeshStandardMaterial({ map: this.texGround(), color: 0x6a7a5e, roughness: 0.95, metalness: 0.05 }));
        ground.position.y = -0.5;
        ground.receiveShadow = true;
        this.world.add(ground);
        // A subtle plaza pad under the central building cluster.
        const plaza = new THREE.Mesh(new THREE.CircleGeometry(11, 48), new THREE.MeshStandardMaterial({ map: this.texConcrete(), color: 0x8c8f96, roughness: 0.9 }));
    ...
    addLamp(x, z) {
        const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 1.6, 8), new THREE.MeshStandardMaterial({ color: 0x2c3038, roughness: 0.6, metalness: 0.5 }));
        pole.position.set(x, 0.8, z);
        pole.castShadow = true;
        this.world.add(pole);
        const head = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.1, 0.3), new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xffe6a0, emissiveIntensity: 1.4 }));
        head.position.set(x, 1.6, z);
        this.world.add(head);
        const glow = makeGlow(0xffe6a0, 1.6);
    ...
    buildBuildings() {
        for (const id of STATION_IDS) {
            ...
            group.add(status);
            const statusGlow = makeGlow(0x66ff88, 1.0);
