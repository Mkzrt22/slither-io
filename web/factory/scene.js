...
export class FactoryScene {
    constructor(canvas, onTapStation) {
        ...
        this.nightMats = [];
        this.stars = null;
        this.dayT = 40; // start mid-morning
        this.night = 0;
        this.daySky = new THREE.Color(0x24344a);
        this.nightSky = new THREE.Color(0x080b12);
    ...
    // --- Ground & roads ---------------------------------------------------------
    buildGround() {
        const ground = new THREE.Mesh(new THREE.BoxGeometry(170, 1, 150), new THREE.MeshStandardMaterial({ map: this.texGround(), color: 0x6a7a5e, roughness: 0.95, metalness: 0.05 }));
        ground.position.y = -0.5;
        ground.receiveShadow = true;
        this.world.add(ground);
        // A subtle plaza pad under the central building cluster.
        const plaza = new THREE.Mesh(new THREE.CircleGeometry(44, 72), new THREE.MeshStandardMaterial({ map: this.texConcrete(), color: 0x82868d, roughness: 0.9 }));
    ...
    buildProps() {
        ...
        for (const [x, z] of [[-36, 22], [36, 22], [-38, -18], [38, -18], [-22, 28], [22, 28], [0, 33], [-40, 5], [40, 5], [-10, 26], [10, 26]]) {
            this.makeTree(x, z);
        }
        // Two parked box trucks in a yard at the back.
        for (const [x, z, rot] of [[-19, -20, 0.3], [-16, -20.5, 0.3], [-22, -19.6, 0.35]]) {
    ...
    buildDistricts() {
        ...
        containers.forEach(([x, z, lvl, rot], i) => {
            const c = new THREE.Mesh(new THREE.BoxGeometry(4.2, 1.7, 1.8), new THREE.MeshStandardMaterial({ color: containerCols[i % containerCols.length], roughness: 0.6, metalness: 0.3 }));
            c.position.set(x, 0.85 + lvl * 1.72, z);
            c.rotation.y = rot;
            c.castShadow = true;
        ...
        for (const [x, z] of [[-36, 17.5], [-36, 11.5]]) {
            ...
            ridge.scale.x = 1.9;
            ridge.position.set(x, 1.95, z);
            this.world.add(ridge);
        }
        for (let r = 0; r < 4; r++) {
            const row = new THREE.Mesh(new THREE.BoxGeometry(4.4, 0.24, 0.7), new THREE.MeshStandardMaterial({ color: r % 2 ? 0x4e8a3c : 0x5f7048, roughness: 1 }));
            row.position.set(-35, 0.12, 21.5 + r * 1.1);
            row.receiveShadow = true;
            this.world.add(row);
        }
        // Cooking: propane tanks + a wood pile feeding the ovens.
        for (const [x, z] of [[-6.5, 27.5], [-4.3, 27.8]]) {
        ...
        for (const [x, z, r] of [[33.5, 17, 0.8], [35, 13.5, 0.6], [32.5, 20, 0.7]]) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            line.position.set(31.8 + i * 2.1, 0.09, -4.8);
            this.world.add(line);
        }
        for (let i = 0; i < 3; i++) {
            const car = this.makeCar();
            car.visible = true;
            car.position.set(32.9 + i * 2.1, 0, -4.8);
            car.rotation.y = 0;
            this.world.add(car);
        }
        ...
        locker.position.set(36, 0.75, 0.6);
        locker.castShadow = true;
        this.world.add(locker);
    }
    ...
    buildScenery() {
        ...
        kerb.position.set(-34, 0.1, 28);
        this.world.add(kerb);
        for (const [x, z] of [[-30, 25], [-38, 30], [-31, 31]]) {
        ...
        for (const [x, z] of [[-9, 30], [-5.5, 31.5], [-12, 32]]) {
            ...
            for (let s = 0; s < 3; s++) {
                ...
                stool.position.set(x + Math.cos(a) * 1.35, 0.31, z + Math.sin(a) * 1.35);
                stool.castShadow = true;
                this.world.add(stool);
            }
        }
        // Solar farm: tilted panel rows powering the campus.
        for (let r = 0; r < 3; r++) {
            for (let c = 0; c < 4; c++) {
                const panel = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.08, 1.4), new THREE.MeshStandardMaterial({ color: 0x1f3a6e, roughness: 0.2, metalness: 0.6 }));
                ...
                post.position.set(30 + c * 2.6, 0.4, 22 + r * 2.2);
                this.world.add(post);
            }
        }
        ...
        ring.rotation.x = -Math.PI / 2;
        ring.position.set(0, 0.06, -18);
        this.world.add(ring);
        const h = this.makeIconSprite('🚁');
        h.scale.set(2, 2, 1);
        h.position.set(0, 1.4, -18);
        this.world.add(h);
        ...
        shelterRoof.position.set(12, 2.2, 28);
        this.world.add(shelterRoof);
        for (const px of [10.6, 13.4]) {
            ...
            post.position.set(px, 1.1, 28.6);
            this.world.add(post);
        }
        this.makeBench(12, 28.4, Math.PI);
        // Woodland clusters at the campus corners.
        for (const [cx, cz] of [[-45, 30], [45, 30], [-45, -25], [45, -25]]) {
            for (let i = 0; i < 5; i++) {
    ...
    buildCenterpiece() {
        ...
        board.position.set(-2.5, 2.9, 1.4);
        g.add(board);
        const logo = this.makeIconSprite('🍔');
        logo.scale.set(0.8, 0.8, 1);
        logo.position.set(-2.5, 2.9, 1.5);
        g.add(logo);
        // Flagpole with a waving flag (animated).
        const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 3.6, 8), steel);
        pole.position.set(2.6, 1.8, -1.6);
        pole.castShadow = true;
        g.add(pole);
    ...
    makeManagerChef(rarity) {
        ...
        hat.add(band);
        w.add(body, head, hat);
        const glow = makeGlow(col, 0.9);
        glow.position.set(0, 1.3, 0);
        glow.material.opacity = 0.5;
        w.add(glow);
        return w;
    }
    syncManagerFigure(vis, managerId) {
        if (managerId === vis.managerId)
            return;
        vis.managerId = managerId;
        if (vis.managerFigure) {
            vis.group.remove(vis.managerFigure);
            vis.managerFigure = null;
        }
        if (managerId) {
    ...
    update(dt) {
        ...
        if (this.centerFlag)
            this.centerFlag.rotation.y = Math.sin(this.t * 4) * 0.25;
        ...
        for (const r of this.roamers) {
            ...
            if (d < 0.12) {
                r.tx = (Math.random() - 0.5) * 48;
                r.tz = 3 + (Math.random() - 0.5) * 40;
    ...
    onPointerMove(e) {
        ...
        if (this.pinching && this.pointers.size >= 2) {
            ...
            if (this.lastPinch > 0 && d > 0)
                this.targetViewSize = Math.max(16, Math.min(95, this.targetViewSize * (this.lastPinch / d)));
            this.lastPinch = d;
        }
