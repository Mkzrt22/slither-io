...
function speckle(g, size, n, colors, min = 1, max = 3) {
    for (let i = 0; i < n; i++) {
...
export function texGrass() {
    return cached('grass', () => {
        ...
        for (let i = 0; i < 80; i++) {
            ...
            g.beginPath();
            g.arc(Math.random() * s, Math.random() * s, 1.4 + Math.random() * 1.4, 0, Math.PI * 2);
            g.fill();
        }
        return finalize(c, 4);
    });
        ...
        for (let i = 0; i < planks; i++) {
            ...
            for (let k = 0; k < 5; k++) {
                ...
                for (let y = 0; y <= s; y += 16)
                    g.lineTo(gx + Math.sin(y * 0.05 + k) * 1.6, y);
                g.stroke();
            }
            // Occasional knot.
            if (Math.random() < 0.55) {
                ...
                g.lineWidth = 1.5;
                g.beginPath();
                g.ellipse(i * w + w / 2, Math.random() * s, 3, 5, 0, 0, Math.PI * 2);
                g.stroke();
            }
...
export function texStoneWall(hex) {
    return cached(`wall_${hex}`, () => {
        const s = 256;
        const { c, g } = canvas(s);
        g.fillStyle = shadeHex(hex, 0.8);
        g.fillRect(0, 0, s, s);
        const bw = 56, bh = 34;
...
export function texShingle(hex) {
    return cached(`shingle_${hex}`, () => {
        const s = 256;
        const { c, g } = canvas(s);
        g.fillStyle = shadeHex(hex, 0.7);
        g.fillRect(0, 0, s, s);
        const rh = 30;
        for (let y = 0, row = 0; y < s; y += rh, row++) {
            ...
            for (let x = -48; x < s; x += 46) {
                ...
                g.moveTo(x + off, y);
                g.lineTo(x + off + 44, y);
                g.lineTo(x + off + 44, y + rh - 3);
                g.arc(x + off + 22, y + rh - 3, 22, 0, Math.PI);
                g.closePath();
                g.fill();
                ...
                g.beginPath();
                g.arc(x + off + 22, y + rh - 3, 22, 0.25, Math.PI - 0.25);
                g.fill();
            }
        }
        return finalize(c, 1);
