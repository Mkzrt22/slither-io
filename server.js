...
const POWERUP_DURATIONS = { speed:5*25, shield:4*25, magnet:8*25 };
const SCORE_WIN = 200;
...
// Wrap-aware squared distance — accounts for world edge wrap-around
function wrapDist2(a, b, W, H) {
...
const GAME_MODES = ['classic','score','teams','tournament'];
...
function mkObstacle(map){
  const r=28+rnd(46);
  return{id:idCounter++,x:400+rnd(map.W-800),y:400+rnd(map.H-800),r,vx:(Math.random()-.5)*.65,vy:(Math.random()-.5)*.65};
}
function mkPortalPair(idx,map){
  const a={id:idCounter++,x:300+rnd(map.W-600),y:300+rnd(map.H-600),r:22,pairIdx:idx,which:'a'};
  const b={id:idCounter++,x:300+rnd(map.W-600),y:300+rnd(map.H-600),r:22,pairIdx:idx,which:'b'};
  return[a,b];
}
...
function mkPlayer(id,name,isBot,skin,team,customSkin,map){
  ...
  return{id,name:isBot?name:(name||'Anonymous').slice(0,16),color,skin:skin||'solid',segs,angle,targetAngle:angle,team:team||null,
    ...
    powerup:null,powerupTick:0,powerupDuration:0,zoneDamageTick:0,_zoneDeath:false,botWanderTick:0,
...
function mkRoom(id,name,mode,mapId){
  ...
  return{id,name:name||'Room '+id.slice(-4),mode:mode||'classic',map,
    ...
    players:{},bots,food,powerups,obstacles,portals,
...
function tickObstacles(room){
  for(const ob of room.obstacles){
    ob.x=((ob.x+ob.vx)%room.W+room.W)%room.W;ob.y=((ob.y+ob.vy)%room.H+room.H)%room.H;
    if(ob.x<200||ob.x>room.W-200)ob.vx*=-1;if(ob.y<200||ob.y>room.H-200)ob.vy*=-1;
  }
}
...
function tryPortal(p,room){
  ...
  for(const portal of room.portals){
    if(wrapDist2(head,portal,room.W,room.H)<(SEG_R+portal.r)**2){
      ...
      p.portalCooldown=60;if(!p.isBot)io.to(p.id).emit('sfx','portal');return;
    }
  }
}
...
function updateBot(bot,room){
  if(!bot.alive){
    bot._respawnTick=(bot._respawnTick||0)+1;
  ...
  const head=bot.segs[0];bot.botWanderTick++;
  if(dist(head,room.zone)>room.zone.r-300){bot.targetAngle=Math.atan2(room.zone.y-head.y,room.zone.x-head.x);return;}
  if(bot.botWanderTick>40+Math.floor(rnd(60))){
    let best=null,bestD=Infinity;
    for(const f of room.food){const d=dist2(head,f);if(d<bestD){bestD=d;best=f;}}
    if(best)bot.targetAngle=Math.atan2(best.y-head.y,best.x-head.x)+(Math.random()-.5)*.4;
    bot.botWanderTick=0;
  }
  bot.boosting=Math.random()<.04;
}
...
function tickRoom(room){
  ...
  for(const p of alive){
    const diff=angleDiff(p.angle,p.targetAngle);p.angle+=Math.max(-TURN_SPEED,Math.min(TURN_SPEED,diff));
    let spd=p.boosting?BOOST_SPEED:MOVE_SPEED;
    if(p.powerup==='speed')spd*=1.9;if(p.abilities.includes('fast_boost')&&p.boosting)spd*=1.2;
    const head=p.segs[0];
    const nx=((head.x+Math.cos(p.angle)*spd)%W+W)%W;const ny=((head.y+Math.sin(p.angle)*spd)%H+H)%H;
    p.segs.unshift({x:nx,y:ny});
    if(p.boosting&&p.segs.length>INIT_SEGS+2){
      p.boostTick++;if(p.boostTick>=BOOST_SHRINK_INTERVAL){p.boostTick=0;const d=p.segs.pop();room.food.push(mkFood(map,d.x,d.y,6,p.color));p.score=Math.max(INIT_SEGS,p.score-1);}else p.segs.pop();
    }else p.segs.pop();
    ...
    for(let i=room.powerups.length-1;i>=0;i--){
      const pu=room.powerups[i];if(wrapDist2({x:nx,y:ny},pu,W,H)<(SEG_R+pu.r)**2){
        room.powerups.splice(i,1);p.powerup=pu.type;p.powerupTick=0;
        let dur=POWERUP_DURATIONS[pu.type]||150;if(pu.type==='shield'&&p.abilities.includes('long_shield'))dur*=2;p.powerupDuration=dur;
        if(!p.isBot){io.to(p.id).emit('sfx','powerup');io.to(p.id).emit('powerup',pu.type);}
      }
    }
    if(p.powerup){p.powerupTick++;if(p.powerupTick>=p.powerupDuration)p.powerup=null;}
    tryPortal(p,room);
    if(dist({x:nx,y:ny},room.zone)>room.zone.r&&p.powerup!=='shield'){
      p.zoneDamageTick++;if(p.zoneDamageTick>=ZONE_DAMAGE_TICK){p.zoneDamageTick=0;if(p.segs.length>8){const d=p.segs.pop();room.food.push(mkFood(map,d.x,d.y,5,p.color));p.score=Math.max(1,p.score-1);}else p._zoneDeath=true;}
  ...
  for (const p of alive) {
    ...
    if (p.powerup === 'shield' || p.abilities.includes('ghost')) continue;
    const head = p.segs[0];

    // Obstacle collision — use 80% of visual radius so hitbox matches polygon
    for (const ob of room.obstacles) {
      if (wrapDist2(head, ob, W, H) < (SEG_R + ob.r * 0.80) ** 2) {
        dead.add(p.id);
        break;
      }
    }
    ...
    for (const other of alive) {
      // Head-to-head fix: don't skip 'other' just because it's already dead —
      // we still want to mark the current player dead too.
      // Only skip body-check of a dead snake (to avoid crediting wrong killer).
      const isSelf = other.id === p.id;

      // Teams: allies don't collide (except self-collision check)
      if (!isSelf && room.mode === 'teams' && p.team && other.team && p.team === other.team) continue;

      const skip     = isSelf ? selfCollisionSkip(p) : 0;
      // Tighter self-collision radius so sharp turns don't false-kill
      const colR2    = isSelf ? (SEG_R * 1.3) ** 2 : (SEG_R * 1.9) ** 2;

      // If other is already dead this tick, only check its head (head-to-head)
      ...
      for (let i = skip; i < checkTo; i++) {
        if (wrapDist2(head, other.segs[i], W, H) < colR2) {
          ...
          if (!isSelf && !dead.has(other.id)) {
            // other is alive and killed p — credit the kill
            other.killCount++;
            other.xp += 30;
  ...
  const now=Date.now();
  room.killFeed=room.killFeed.filter(k=>now-k.t<5000);room.mapPings=room.mapPings.filter(k=>now-k.t<3000);room.chatMsgs=room.chatMsgs.filter(k=>now-k.t<8000);

  const leaderboard=Object.values(all).sort((a,b)=>b.score-a.score).slice(0,10).map(p=>({name:p.name,score:p.score,alive:p.alive,kills:p.killCount,isBot:p.isBot,color:p.color,team:p.team,level:p.level}));
  const pArr=Object.values(all).filter(p=>p.alive).map(p=>({id:p.id,name:p.name,color:p.color,skin:p.skin,customSkin:p.customSkin,segs:p.segs,score:p.score,boosting:p.boosting,powerup:p.powerup,powerupTick:p.powerupTick,powerupDuration:p.powerupDuration||1,isBot:p.isBot,team:p.team,level:p.level,xp:p.xp,abilities:p.abilities,emote:p.emote}));

  io.to(room.id).emit('state',{players:pArr,food:room.food,powerups:room.powerups,obstacles:room.obstacles,portals:room.portals,leaderboard,killFeed:room.killFeed,zone:room.zone,mode:room.mode,teamScores:room.teamScores,scoreWin:SCORE_WIN,chatMsgs:room.chatMsgs,mapPings:room.mapPings,mapId:room.map.id,W,H});
}
...
io.on('connection',socket=>{
  ...
  socket.on('createRoom',({name,roomName,skin,mode,mapId,customSkin})=>{
    ...
    if(m==='classic'||m==='score')startZone(room);
  ...
  socket.on('input',({angle,boosting})=>{const r=rooms[socketToRoom[socket.id]];const p=r?.players[socket.id];if(!p||!p.alive)return;p.targetAngle=angle;p.boosting=boosting;});
...
function joinRoom(socket,room,name,skin,mode,customSkin){
  ...
  socket.emit('joined',{id:socket.id,W:room.W,H:room.H,roomId:room.id,roomName:room.name,mode:room.mode,team,mapId:room.map.id});
}
...
let stripe = null;
if (STRIPE_SECRET) {
  try { stripe = require('stripe')(STRIPE_SECRET); } catch(e) { console.warn('Stripe not available:', e.message); }
}
...
// Create a Checkout Session → returns {url} to redirect to
...
// Verify a completed session → return a signed unlock token the client stores in localStorage
// No database needed: the token is HMAC-signed so it can't be forged
app.get('/api/verify-purchase', async (req, res) => {
  ...
  try {
    ...
    // Sign a token binding the session ID — client stores this as proof of purchase
    const payload = `supporter:${session.id}`;
    const sig = crypto.createHmac('sha256', TOKEN_SECRET).update(payload).digest('hex');
    res.json({ token: `${payload}:${sig}`, product: session.metadata.product });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});

// Lightweight token check (no Stripe call) — just validates HMAC
...
app.post('/api/stripe-webhook', (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    console.log('✅ Purchase confirmed:', event.data.object.id);
  }
  res.sendStatus(200);
});

app.use(express.static(path.join(__dirname,'public')));
server.listen(process.env.PORT||3000,()=>console.log('🐍 Slither v5 → http://localhost:'+(process.env.PORT||3000)));
