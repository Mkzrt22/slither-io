...
const fmtRate = (perSec: number): string => `${fmt(perSec * 60)}/min`;
...
function makeStore(): KeyValueStore {
  try {
    const k = '__probe__'; window.localStorage.setItem(k, '1'); window.localStorage.removeItem(k);
...
// Scene.
const host = el<HTMLDivElement>('factory-host');
...
let sheetMode: BuyMode = 1;
let manageMode: BuyMode = 1;
bindBuyModes('buy-modes', (m) => { sheetMode = m; });
bindBuyModes('manage-modes', (m) => { manageMode = m; });
function bindBuyModes(id: string, set: (m: BuyMode) => void): void {
  const btns = Array.from(document.querySelectorAll<HTMLButtonElement>(`#${id} button`));
  for (const b of btns) {
    b.addEventListener('click', () => {
      set(b.dataset.mode === 'max' ? 'max' : (Number(b.dataset.mode) as 1 | 10));
      for (const o of btns) o.classList.toggle('active', o === b);
    });
  }
}

// --- Station management list (Gestion tab) -----------------------------------

interface StationRow {
  root: HTMLElement; level: HTMLElement; rate: HTMLElement; buffer: HTMLElement;
  workers: HTMLElement; buyLbl: HTMLElement; cost: HTMLElement; buyBtn: HTMLButtonElement;
}
const stationRows = new Map<StationId, StationRow>();
const stationListEl = el<HTMLDivElement>('station-list');
for (const def of STATION_DEFS) {
  ...
  (stationRows.get(def.id) as unknown as { bar: HTMLElement }).bar =
...
interface RecipeCard { root: HTMLElement; btn: HTMLButtonElement; }
const recipeCards = new Map<RecipeId, RecipeCard>();
const recipeStripEl = el<HTMLDivElement>('recipe-strip');
...
function renderRecipes(s: FactoryState): void {
  for (const def of RECIPE_DEFS) {
    ...
    if (active) { c.btn.textContent = '✓ En production'; c.btn.disabled = true; c.btn.className = 'rp-btn active'; }
    else if (unlocked) { c.btn.textContent = 'Produire'; c.btn.disabled = false; c.btn.className = 'rp-btn'; }
    else {
...
interface ResearchRow { level: HTMLElement; effect: HTMLElement; buyBtn: HTMLButtonElement; }
const researchRows = new Map<string, ResearchRow>();
const researchListEl = el<HTMLDivElement>('research-list');
...
let openStation: StationId | null = null;
const stationSheet = el<HTMLDivElement>('station-sheet');
function openStationSheet(id: StationId): void {
  openStation = id;
...
function closeStationSheet(): void {
  openStation = null;
...
for (const btn of tabButtons) {
  btn.addEventListener('click', () => {
    ...
    if (btn.dataset.tab === 'tab-factory') requestAnimationFrame(() => scene?.resize());
  });
}
...
let pendingDaily: { streak: number; cashReward: number; gemReward: number } | null = null;
...
game.on('offline', (r) => {
  ...
  el<HTMLButtonElement>('btn-offline-x2').hidden = r.cash <= 0;
  el('offline-modal').classList.add('visible');
});
game.on('daily', (d) => {
  pendingDaily = d;
  if (el('offline-modal').classList.contains('visible')) return;
  showDaily();
});
...
el<HTMLButtonElement>('btn-offline-x2').addEventListener('click', async () => {
  const btn = el<HTMLButtonElement>('btn-offline-x2'); btn.disabled = true;
  await new Promise((r) => setTimeout(r, 400));
  if (pendingOfflineCash > 0) game.grantBonusCash(pendingOfflineCash);
  btn.disabled = false;
  el('offline-modal').classList.remove('visible');
  showDaily();
});
...
el('btn-export').addEventListener('click', () => {
  ...
  void navigator.clipboard?.writeText(code).catch(() => { /* optional */ });
...
el('btn-import').addEventListener('click', () => {
  ...
  if (code && game.importSave(code)) { window.alert('Importé ! Rechargement…'); location.reload(); }
});
el('btn-reset').addEventListener('click', () => {
  if (window.confirm('Réinitialiser toute la progression ?')) { game.reset(); location.reload(); }
});

// --- Rendering ---------------------------------------------------------------

let firstRender = true;
function render(s: FactoryState): void {
  const now = Date.now();
  ...
  if (isActive('tab-manage')) { renderRecipes(s); renderManage(s, now); }
  if (isActive('tab-research')) renderResearch(s);
  if (isActive('tab-prestige')) renderPrestige(s);
  if (openStation) renderStationSheet(s);
...
function renderManage(s: FactoryState, now: number): void {
  for (const id of STATION_IDS) {
    ...
    (row as unknown as { bar: HTMLElement }).bar.style.width = `${Math.round(fill * 100)}%`;
  ...
  (menuBtn as HTMLButtonElement).disabled = s.cash < menuCost;
}

function renderResearch(s: FactoryState): void {
  for (const def of RESEARCH_DEFS) {
    ...
    if (cost === null) {
      row.buyBtn.textContent = 'MAX'; row.buyBtn.disabled = true;
    } else {
...
function renderPrestige(s: FactoryState): void {
  ...
  el('prestige-fill').style.width = `${Math.min(100, (s.stats.cashRun / 1_000_000) * 100)}%`;
  const btn = el<HTMLButtonElement>('btn-prestige');
  ...
  el('prestige-note').textContent = pending > 0
    ...
    : `Encaissez ${fmt(1_000_000)} € dans ce cycle (actuel : ${fmt(s.stats.cashRun)}).`;
...
function renderStationSheet(s: FactoryState): void {
  if (!openStation) return;
  ...
  const upgBtn = el<HTMLButtonElement>('btn-upgrade-station');
  ...
  el<HTMLButtonElement>('btn-worker-plus').disabled = s.workersIdle < 1 || st.workers >= WORKERS_PER_STATION_CAP;
  el<HTMLButtonElement>('btn-worker-minus').disabled = st.workers < 1;
...
function localScore(): number { return Math.floor(game.getState().stats.cashAll || 0); }
async function pushCloud(keepalive = false): Promise<void> {
  if (!cloud.enabled()) return;
  const score = localScore();
  if (score === lastPushedScore && !keepalive) return;
  const res = await cloud.push(game.exportSave(), keepalive);
  if (res.status === 'ok') lastPushedScore = score;
  else if (res.status === 'stale' && res.snapshot.code && res.snapshot.score > score) {
    if (game.importSave(res.snapshot.code)) location.reload();
  }
}
async function initCloud(): Promise<void> {
  ...
  if (snap && snap.code && snap.score > localScore()) {
    if (game.importSave(snap.code)) { location.reload(); return; }
  }
  await pushCloud();
  window.setInterval(() => { void pushCloud(); }, 30_000);
}
document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') void pushCloud(true); });
