...
interface BuildingPalette { top: string; topLit: string; left: string; right: string; roof: string; roofDark: string; }
...
const LAYOUT: Record<BuildingType, Vec> = {
  mine:       { x: 1, y: 1 },
  farm:       { x: 4, y: 1 },
  sawmill:    { x: 1, y: 4 },
  market:     { x: 3.2, y: 3.2 },
...
interface Worker { x: number; y: number; tx: number; ty: number; speed: number; pause: number; hue: string; }
interface Coin { sx: number; sy: number; vy: number; life: number; }
...
export class IsoScene {
  ...
  private raf = 0;
  private last = 0;
  private levels: Record<BuildingType, number> = {
    mine: 0, farm: 0, sawmill: 0, market: 0, blacksmith: 0, castle: 0,
  };
  private workers: Worker[] = [];
  private coins: Coin[] = [];
  private hitboxes: { type: BuildingType; poly: Vec[] }[] = [];
  private t = 0;
  ...
  public setState(state: UserProfile): void {
    ...
    while (this.workers.length < target) this.spawnWorkers(1);
    if (this.workers.length > target) this.workers.length = target;
    this.ensureRunning();
  }
  ...
  private heightOf(type: BuildingType): number {
    const base = type === 'castle' ? 30 : 16;
    return base + Math.min(this.levels[type], 24) * 2.6;
  }

  private toScreen(gx: number, gy: number, gz: number): Vec {
    return {
  ...
  private update(dt: number): void {
    ...
    for (const c of this.coins) { c.life += dt; c.sy += c.vy * dt; c.vy += 60 * dt; }
  ...
  private draw(): void {
    ...
    this.hitboxes = [];
    interface Drawable { depth: number; render: () => void; }
  ...
  private drawBuilding(type: BuildingType): void {
    const ctx = this.ctx;
    const b = LAYOUT[type];
    ...
    ctx.beginPath();
    ctx.moveTo(Tr.x, Tr.y); ctx.lineTo(Tf.x, Tf.y); ctx.lineTo(Bf.x, Bf.y); ctx.lineTo(Br.x, Br.y);
    ctx.closePath(); ctx.fill();
    // left face
    ...
    ctx.beginPath();
    ctx.moveTo(Tb.x, Tb.y); ctx.lineTo(Tr.x, Tr.y); ctx.lineTo(Tf.x, Tf.y); ctx.lineTo(Tl.x, Tl.y);
    ctx.closePath(); ctx.fill();
    ...
    ctx.beginPath(); ctx.moveTo(Rr.x, Rr.y); ctx.lineTo(Rf.x, Rf.y); ctx.lineTo(Apex.x, Apex.y); ctx.closePath(); ctx.fill();
    ctx.fillStyle = pal.roof;
    ctx.beginPath(); ctx.moveTo(Rb.x, Rb.y); ctx.lineTo(Rr.x, Rr.y); ctx.lineTo(Apex.x, Apex.y); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(Rl.x, Rl.y); ctx.lineTo(Rb.x, Rb.y); ctx.lineTo(Apex.x, Apex.y); ctx.closePath(); ctx.fill();
    ctx.fillStyle = pal.roofDark;
    ctx.beginPath(); ctx.moveTo(Rf.x, Rf.y); ctx.lineTo(Rl.x, Rl.y); ctx.lineTo(Apex.x, Apex.y); ctx.closePath(); ctx.fill();

    // floating icon banner above
    this.label(Apex.x, Apex.y - 12, BUILDING_CONFIGS[type].icon);
    // level pip
    ctx.fillStyle = 'rgba(0,0,0,0.6)';
    ctx.beginPath(); ctx.arc(Apex.x + 12, Apex.y - 10, 8, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#ffe08a';
  ...
  private drawWorker(w: Worker): void {
    ...
    ctx.beginPath(); ctx.arc(bx, by - 7, 3, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  }
...
function pointInPoly(px: number, py: number, poly: Vec[]): boolean {
  let inside = false;
