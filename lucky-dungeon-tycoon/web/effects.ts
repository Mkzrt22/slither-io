...
import { SlotSymbol } from '../src/types.js';
...
type ParticleKind = 'coin' | 'confetti' | 'spark' | 'text' | 'gem';
...
export class ParticleSystem {
  ...
  public burstCoins(x: number, y: number, count: number): void {
  ...
  public burstGems(x: number, y: number, count: number): void {
    for (let i = 0; i < count; i++) {
  ...
  public confetti(count: number): void {
  ...
  public sparks(x: number, y: number, count: number, color = '#fff3c4'): void {
    for (let i = 0; i < count; i++) {
  ...
  public floatText(x: number, y: number, text: string, color: string, big = false): void {
    this.particles.push({
      kind: 'text',
      x,
      y,
  ...
  private drawCoin(p: Particle): void {
    const ctx = this.ctx;
    ctx.save();
    ctx.translate(p.x, p.y);
    ctx.rotate(p.rot);
...
export class NumberTween {
  ...
  public set(value: number, animate = true): void {
...
const ALL_SYMBOLS: SlotSymbol[] = ['COIN', 'BAG', 'GEM', 'SHIELD', 'SWORD', 'SKULL'];
...
export class SlotReels {
  ...
  constructor(
    ...
    private readonly cellHtml: (s: SlotSymbol) => string,
    private readonly cellPx: number,
  ) {
    this.cols = Array.from(container.querySelectorAll<HTMLElement>('.reel-col'));
    this.strips = this.cols.map((c) => c.querySelector<HTMLElement>('.reel-strip')!);
    for (let i = 0; i < this.strips.length; i++) {
      this.renderStrip(i, this.randomCells(6, ALL_SYMBOLS[i % ALL_SYMBOLS.length]));
    }
  }

  private randomCells(n: number, last: SlotSymbol): SlotSymbol[] {
  ...
  private renderStrip(i: number, cells: SlotSymbol[]): void {
