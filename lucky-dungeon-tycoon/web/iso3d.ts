...
import {
  makeGlow, texCobble, texGrass, texPlanks, texShingle, texStone, texStoneWall,
} from './iso3dtex.js';
...
/**
 * A delivery scooter shuttling food between the plaza hub and an eatery. It
 * drives out with a glowing food box, pauses at each end, then heads back —
 * the genre's signature "things moving along the routes" bustle.
 */
interface Scooter3D {
  mesh: THREE.Group;
  hub: THREE.Vector3; dest: THREE.Vector3;
  leg: 'out' | 'back';
  t: number; speed: number; pause: number;
}
...
export class Iso3DScene {
  ...
  private buildPaths(): void {
    ...
    for (const type of BUILDING_TYPES) {
      ...
      const tex = texCobble().clone(); tex.needsUpdate = true;
  ...
  private scatterProps(): void {
    const blocked = (x: number, z: number): boolean => {
      ...
      if (Math.hypot(x - (HALF + 1.6), z - (-HALF - 1.6)) < 2.3) return true; // pond
      if (Math.hypot(x - (HALF + 1.7), z - (HALF + 1.7)) < 2.3) return true;  // fountain
      return false;
    };
    for (let gx = 0.5; gx <= GRID - 0.5; gx += 1) {
      for (let gy = 0.5; gy <= GRID - 0.5; gy += 1) {
        ...
        if (blocked(px, pz) || Math.random() < 0.42) continue;
        const pick = Math.random();
        if (pick < 0.38) this.makeBush(px, pz);
        else if (pick < 0.66) this.makeFlowerBed(px, pz);
        else if (pick < 0.82) this.makeRock(px, pz);
        else if (pick < 0.93) this.makeCrate(px, pz);
        else this.makeBarrel(px, pz);
      }
    }
  ...
  private makeBush(x: number, z: number): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      blob.castShadow = true; bush.add(blob);
    }
    ...
    bush.position.set(x, -0.7, z); bush.scale.setScalar(0.85 + Math.random() * 0.5);
    this.world.add(bush);
  }

  private makeFlowerBed(x: number, z: number): void {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      stem.position.set(fx, 0.15, fz); bed.add(stem);
      ...
      bloom.position.set(fx, 0.32, fz); bloom.scale.y = 0.72; bed.add(bloom);
    }
    bed.position.set(x, -0.7, z);
    this.world.add(bed);
  }

  private makeRock(x: number, z: number): void {
    ...
    rock.scale.set(1, 0.7, 1); rock.castShadow = true; rock.receiveShadow = true;
    this.world.add(rock);
  }

  private makeCrate(x: number, z: number): void {
    ...
    crate.position.set(x, -0.45, z); crate.rotation.y = Math.random() * Math.PI;
    crate.castShadow = true; crate.receiveShadow = true; this.world.add(crate);
    if (Math.random() < 0.4) {
      const c2 = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.4, 0.4), mat);
      c2.position.set(x + 0.32, -0.5, z + 0.22); c2.rotation.y = Math.random() * Math.PI;
      c2.castShadow = true; this.world.add(c2);
    }
  }

  private makeBarrel(x: number, z: number): void {
    ...
    body.position.y = 0.27; body.castShadow = true; barrel.add(body);
    ...
    barrel.position.set(x, -0.7, z); barrel.rotation.y = Math.random() * Math.PI;
  ...
  private makeBench(x: number, z: number, ry: number): void {
    ...
    seat.position.y = 0.26; seat.castShadow = true; bench.add(seat);
    const back = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.24, 0.05), mat);
    back.position.set(0, 0.4, -0.11); back.castShadow = true; bench.add(back);
    for (const lx of [-0.34, 0.34]) {
      const leg = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.26, 0.24), mat);
      leg.position.set(lx, 0.13, 0); bench.add(leg);
    }
    bench.position.set(x, -0.7, z); bench.rotation.y = ry; this.world.add(bench);
  }
   ...
   */
  private makeTerrace(x: number, z: number): void {
    ...
    canopy.position.y = 1.42; canopy.castShadow = true; terrace.add(canopy);
    ...
    for (let i = 0; i < seats; i++) {
      ...
      stool.position.set(sx, 0.16, sz); stool.castShadow = true; terrace.add(stool);
      if (i < 2) {
        const diner = new THREE.Mesh(
          new THREE.CapsuleGeometry(0.12, 0.22, 4, 8),
          new THREE.MeshStandardMaterial({ color: new THREE.Color().setHSL((i * 0.37 + 0.1) % 1, 0.5, 0.55), roughness: 0.8 }),
        );
        diner.position.set(sx, 0.5, sz); diner.castShadow = true; terrace.add(diner);
        const head = new THREE.Mesh(
          new THREE.SphereGeometry(0.1, 12, 10),
          new THREE.MeshStandardMaterial({ color: 0xf1d3a8, roughness: 0.7 }),
        );
        head.position.set(sx, 0.74, sz); terrace.add(head);
      }
    }
    ...
    plate.position.y = 0.55; terrace.add(plate);
    this.nightMats.push(plateMat);
    terrace.position.set(x, -0.7, z); terrace.rotation.y = Math.random() * Math.PI * 2;
  ...
  private makeScooterMesh(): THREE.Group {
    ...
    deck.position.y = 0.24; deck.castShadow = true; g.add(deck);
    const front = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.34, 0.12), bodyMat);
    front.position.set(0, 0.36, 0.3); front.castShadow = true; g.add(front);
    ...
    box.position.set(0, 0.46, -0.28); box.castShadow = true; g.add(box);
    this.nightMats.push(boxMat); // the box glows warmer at dusk
    const rider = new THREE.Mesh(
      new THREE.CapsuleGeometry(0.1, 0.2, 4, 8),
      new THREE.MeshStandardMaterial({ color: 0x2f55c8, roughness: 0.7 }),
    );
    rider.position.set(0, 0.5, 0.04); rider.castShadow = true; g.add(rider);
    const helmet = new THREE.Mesh(
      new THREE.SphereGeometry(0.11, 12, 10),
      new THREE.MeshStandardMaterial({ color: 0xf2f2f2, roughness: 0.4 }),
    );
    helmet.position.set(0, 0.72, 0.04); g.add(helmet);
  ...
  private makeScooter(dest: BuildingType): Scooter3D {
  ...
  private refreshScooters(built: BuildingType[]): void {
    ...
    while (this.scooters.length > want) {
      const s = this.scooters.pop(); if (s) this.world.remove(s.mesh);
    }
    while (this.scooters.length < want) {
  ...
  private spawnSmoke(): void {
    const kitchens: BuildingType[] = ['sawmill', 'market', 'blacksmith', 'castle'];
    ...
    puff.position.set(
      b.group.position.x + (Math.random() - 0.5) * 0.8,
      b.shownHeight + 1.3,
      b.group.position.z - 0.3,
    );
    this.world.add(puff);
  ...
  private update(dt: number): void {
    ...
    for (const s of this.scooters) {
      ...
      if (s.t >= 1) {
        s.t = 0; s.leg = s.leg === 'out' ? 'back' : 'out';
