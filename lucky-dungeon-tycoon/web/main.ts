...
interface VillageRenderer {
  ...
  /** Optional: float a "+income" number rising from a building (idle juice). */
  floatIncome?(type: BuildingType, text: string): void;
}
...
setInterval(() => {
  if (!iso.floatIncome) return;
  if (!document.getElementById('tab-village')?.classList.contains('active')) return;
  ...
  let pick = built[0];
  for (let i = 0; i < built.length; i++) { r -= weights[i]; if (r <= 0) { pick = built[i]; break; } }
