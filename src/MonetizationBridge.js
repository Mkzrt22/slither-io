 ...
 */
import { cloneProfile } from './types.js';
/** Simulated SDK round-trip latency in milliseconds. */
const AD_SIMULATED_LATENCY_MS = 1000;
...
export class MonetizationBridge {
     ...
     */
    static buyEnergyWithGems(state) {
        if (!Number.isFinite(state.gems) || state.gems < ENERGY_PACK_GEM_COST) {
            throw new Error(`INSUFFICIENT_GEMS: ${ENERGY_PACK_GEM_COST} gems required, ` +
